    ...
    <FeedLayout>
        ...
        >
            <div class="flex items-center justify-between h-14 px-4 pointer-events-auto safe-area-top">
                ...
                <router-link
                    :to="`/search?q=${encodeURIComponent(query)}&tab=videos`"
                ...
                >
                    ...
                    <span class="text-white text-sm font-semibold truncate max-w-[200px]">{{ query }}</span>
                </router-link>
...
import { useSearchFeed } from '~/composables/useSearchFeed'
...
const goBack = () => {
    if (window.history.length > 1) {
        ...
        router.push(`/search?q=${encodeURIComponent(query.value)}&tab=videos`)
    }
}

const feedData = useSearchFeed(query)
